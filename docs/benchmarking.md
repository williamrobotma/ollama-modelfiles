...
Each suite's `run.sh` sets its suite name and sources `common.sh`. `matrix.tsv` lists the model IDs to compare; `runtime.tsv` defines the runtime A/B profiles (graphs-off vs graphs-on).
...
- `127.0.0.1:11435` - graphs-off profile (the prod target: `GGML_CUDA_DISABLE_GRAPHS=1`).
- `127.0.0.1:11436` - graphs-on profile (`GGML_CUDA_DISABLE_GRAPHS` unset).
- `127.0.0.1:11437` - `scripts/repro-mtp-graphs.sh` only.

All three suites reuse 11435/11436, so **never run two suites concurrently** unless you change the host assignments; `all.sh` is sequential and safe. For the cleanest results, stop or idle the systemd Ollama service first.
