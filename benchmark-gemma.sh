...
matrix_file="$script_dir/benchmark-gemma.matrix.tsv"
runtime_file="$script_dir/benchmark-gemma.runtime.tsv"
...
usage() {
    cat <<'EOF'
Usage: ./benchmark-gemma.sh [options]

Set up or run a fixed Gemma benchmark matrix against Ollama models.

The benchmark matrix is evaluated across isolated runtime profiles defined in
benchmark-gemma.runtime.tsv. The default profiles A/B the current
...
prompt_files() {
    local prompt
    shopt -s nullglob
    for prompt in "$script_dir"/benchmark-gemma.prompt.*.txt; do
        printf '%s\n' "$prompt"
    done | sort
    shopt -u nullglob
}
...
prompt_name() {
    ...
    local base
    base="${prompt_file##*/}"
    base="${base#benchmark-gemma.prompt.}"
    printf '%s\n' "${base%.txt}"
}
...
if [[ "$execute" -eq 0 ]]; then
    ...
    cp "$matrix_file" "$output_dir/benchmark-gemma.matrix.tsv"
    cp "$runtime_file" "$output_dir/benchmark-gemma.runtime.tsv"
