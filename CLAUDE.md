...
Modelfile.gemma4-12b-it-uncensored     — Uncensored Gemma 4 12B (DuoNeural abliterated), resident, thinking
Modelfile.gemma4-12b-it-obliterated    — Uncensored Gemma 4 12B (OBLITERATUS CoT-aware abliterated), resident; A/B vs DuoNeural
Modelfile.gemma4-26b-a4b-it-heretic-q5-k-m — Uncensored Gemma 4 26B-A4B MoE (Heretic ARA, i1-Q5_K_M), offload; reasoning/agentic daily driver
...
- **Uncensored / reasoning variants** (separate non-coding track: reasoning/research/agentic): daily driver `gemma4-26b-a4b-it-heretic-q5-k-m` (Gemma-4-26B-A4B MoE ~3.8B active, Heretic/ARA-abliterated, mradermacher i1-Q5_K_M ~19 GB, offload; verified Claude-Code-safe + reasoning intact). Resident 12B options: `gemma4-12b-it-uncensored` (DuoNeural) and `gemma4-12b-it-obliterated` (OBLITERATUS/Pliny CoT-aware). Qwen vanilla-vs-distill A/B: `qwen3.6-35b-a3b-abliterated-q4-k` (huihui plain) and `qwen3.6-35b-a3b-claude4.7-abliterated-q4-k` (huihui Claude-4.7 distill). All reference **community abliterated GGUFs** (not Unsloth, so quant tags are plain Q4/Q5_K, not `UD-*`) and use Gemma `<|think|>` / Qwen thinking. Abliteration can dent reasoning/tool-calling - verify on-task (the "ultra"/higher-KL builds most); all must pass the Claude-Code template gate below. See `session_summary_uncensored_models.md`.
- **Claude-Code compatibility gate for community GGUFs**: claude-local drives Ollama via `ollama launch claude` -> Ollama's Anthropic endpoint. Claude Code sends **multiple `system`-role messages mid-conversation** (top-level system + SessionStart hook + skills/system-reminders). A model's embedded Jinja `chat_template` must tolerate non-first/multiple system messages. The removed `qwen3.6-35b-a3b-uncensored-q4-k-m` (HauhauCS abliterated) shipped a non-standard guard (`raise_exception('System message must be at the beginning.')` that canonical Qwen3.6 lacks), so every Claude Code request 400'd; deleted for that reason. Ollama's Modelfile `TEMPLATE` is Go-only (legacy `/api/generate`) and cannot override the GGUF's Jinja template, so such a guard can only be fixed by patching the GGUF metadata. Vet a community GGUF's `chat_template` (`ollama show --template`) before adding it as a Claude Code target.
