...
How the local-LLM stack fits together after the 2026-07 migration to hf-downloaded local GGUFs (see `session_summary_migration_local_ggufs.md` for the migration evidence log).
...
```text
   ...
   Modelfile.* --> FROM /abs/snapshot/path.gguf     ./llama-server --model <same path>
          |                                         (b9860 CUDA build in ~/Developer/llama.cpp;
          |  ./ollama-create.sh                      NOT usable for Gemma MTP: bug #24795)
          v
...
Key property: the repo's Modelfiles are the *only* mapping layer. Every installed Ollama model is reproducible from `git clone` + `hf download` + `./ollama-create.sh`, and the same cached GGUFs feed llama.cpp without conversion.

## 2. Modelfile layering (inside the repo)

```text
CANONICAL (quant-suffixed, full param block)
  Modelfile.gemma4-26b-a4b-it-qat
...
LAYERED / DERIVED (FROM a local model name)
  Modelfile.gemma4-26b-a4b-it-qat-mtp
  ...
  Modelfile.qwen3.6-35b-a3b-mtp-coding-ud-q5-k-xl
...
THIN ALIASES (one line, repoint-able defaults)
  Modelfile.qwen3.6-35b-a3b-mtp-coding
      FROM qwen3.6-35b-a3b-mtp-coding-ud-q5-k-xl
```

`ollama-create.sh` resolves this graph bottom-up (canonical -> layered -> alias).
...
- Rebuild any model: `./ollama-create.sh Modelfile.<name>` (source bytes already cached; create dedups by sha256 -> instant re-link).
- Re-provision from nothing: `hf download` the repos listed in the FROM paths -> same pinned snapshots -> `./ollama-create.sh`.
- Session state: `.task_plan.md` (resume point), `session_summary_migration_local_ggufs.md` (full evidence log), `.migration-artifacts/` (git-excluded: baselines of the pre-migration store, HF inventories, the preseed/migrate/validate scripts).
